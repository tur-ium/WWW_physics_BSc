...
'''
1) Set generatedDataFilePath to directory which contains (the 5 folders: p=0, p=0.25, p=0.5, p=0.75, p=1)
...
def plotData(p, plotACC=True, plotLCS=True, plotErrors=True, plotSTD=True):
    ...
    if(plotSTD):
        if(plotACC):
            plt.figure()
            plt.grid()
            plt.xlabel("Number of Wall Posts, m")
        ...
        if(plotLCS):
            ...
            plt.show()
'''        
def dataCollapse(LCS_ADD, LCS_REMOVE, ACC_ADD, ACC_REMOVE, p, collapseACC=True, collapseLCS=True):
     x = (np.array(list(range(len(AVG_LCS_ADD))))+1)*stepSize #1-1332 (Multiples of 50 (edges added))
     xr = np.array(list(x)[::-1])
     
     maxWeight = x[-1]
     
     normalized_LCS_ADD = np.array(LCS_ADD)/NMAX
     normalized_LCS_REMOVE = np.array(LCS_REMOVE)/NMAX
     normalized_ACC_ADD = np.array(ACC_ADD)/1
     normalized_ACC_REMOVE = np.array(ACC_REMOVE)/1
     
     normalizedX = x/maxWeight
     normalizedXr = xr/maxWeight
     
     
     return normalizedX, normalizedXr, normalized_LCS_ADD, normalized_LCS_REMOVE, normalized_ACC_ADD, normalized_ACC_REMOVE
     
     


LCS_ADD, LCS_REMOVE, ACC_ADD, ACC_REMOVE = collateData(p, noOfFiles=19)
