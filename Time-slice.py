...
def plotInteractionsPerMonth(startPeriod, endPeriod):
    ...
    for date in xTicks:
        ...
        year = monthYear[0]
        month = monthYear[1]
        interactionCount = len(open(filePath_text+str(year)+"/{}:{}.txt".format(year, month), mode='r').readlines())
...
#timeSlicedEdgeList = timeSliceEdges(edgeList, TIME_DICT["2004"]["DEC"], TIME_DICT["2005"]["JAN"], retainWeights=False)
#G = generateTimeSliceGraph(timeSlicedEdgeList)
#createGEXFFromGraph(G, filePath_gexf_2004, "2004:DEC:MultiDiGraph.gexf")
#writeEdgeListToFile(timeSlicedEdgeList, filePath_text_2004, "2004:DEC.txt")


plotInteractionsPerMonth([2004, 'DEC'],[2006, 'JAN'])
