...
def computeLCS(G):
    '''Computes largest component size on the graph G. WARNING: This function was changed on 11th March 2018. '''
    #return len(max(nx.connected_components(G),key=len))
    return max([len(comp) for comp in nx.connected_components(G)])
